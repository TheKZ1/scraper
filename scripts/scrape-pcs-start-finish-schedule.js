...
// Returns true for MU (Under-23 men) and MJ (Junior men) races — these are excluded.
function isJuniorOrU23MenRaceText(value) {
  const norm = normalizeNameForCompare(value);
  if (!norm) return false;
  return /\b(mj|mu)\b/i.test(norm);
}
...
async function loadRaceMap(rows) {
  ...
  (data || [])
    ...
    .filter((race) => !isJuniorOrU23MenRaceText(`${race.name || ''} ${race.slug || ''}`))
    .forEach((race) => {
...
function resolveRaceForRow(row, context) {
  ...
  const rowLooksJuniorOrU23Men = isJuniorOrU23MenRaceText(`${row.raceName || ''} ${row.slug || ''}`);
 
  // Skip women's and MU/MJ rows — we only process elite men's races.
  if (rowLooksWomen) return { race: null, method: 'women-row-skipped' };
  if (rowLooksJuniorOrU23Men) return { race: null, method: 'junior-u23-men-row-skipped' };
